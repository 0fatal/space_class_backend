...
module.exports = {
    initRouter(r) {
        ...
        r.get('/profile/info', UserApi.getUserInfo) // 获取个人信息
        {
            ...
            r.post('/class/info', ClassApi.getClassInfo) // 获取班级信息
            r.post(
                '/class/info/update',
                ClassApi.updateClassInfo,
                AuthMiddleware.NeedTeacher
            ) // 更新班级信息
            r.post(
                '/class/dismiss',
                ClassApi.dismissClass,
                AuthMiddleware.NeedTeacher
            ) // 解散班级
            r.post(
                '/class/create',
...

        {
            r.get(
                '/teacher/class/list',
                ClassApi.getTeacherClassList,
                AuthMiddleware.NeedTeacher
            ) // 获取老师自己的班级列表
            r.post('/teacher/info', TeacherApi.getTeacherInfo) // 获取老师信息
        }
...

        {
            ...
            r.post('/resource', ResourceApi.getResourceDetail) // 班级资源详情
        }

        {
            r.post('/student/list', StudentApi.getStudentList) // 班级学生列表
            r.post('/student/info', StudentApi.getStudentInfo) // 学生信息
        }

        {
            ...
            r.post('/notice/detail', NoticeApi.getNoticeDetail) // 通知详情
            r.post('/notice/list', NoticeApi.getNoticeList) // 获取通知列表
        }

        r.post(
            '/apply/create/join',
            ManageApi.createJoinApply,
            AuthMiddleware.NeedStudent
        ) // 创建入班申请
        r.post(
            '/apply/create/leave',
            ManageApi.createLeaveApply,
            AuthMiddleware.NeedStudent
        ) // 创建退班申请

        {
            ...
            r.post(
                '/manage/apply/ignore',
                ManageApi.ignoreApply,
                AuthMiddleware.NeedTeacher
            ) // 忽略申请
            r.post(
                '/manage/apply/agree',
                ManageApi.agreeApply,
                AuthMiddleware.NeedTeacher
            ) // 同意加入班级申请
            r.post(
                '/manage/remove',
                ManageApi.removeStudent,
                AuthMiddleware.NeedTeacher
            ) // 移出班级
        }
    },
